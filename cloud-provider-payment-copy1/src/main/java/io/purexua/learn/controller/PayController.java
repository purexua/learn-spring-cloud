...
public class PayController {
  ...
  public ResultData<List<Pay>> getAllPay() {
    ...
    return ResultData.success(payService.getAllPay());
  }

  @GetMapping("module/info")
  public ResultData<String> getMessage(@Value("${user.username}") String username, @Value("${user.from}") String from, @Value("${server.port}") String port) {
